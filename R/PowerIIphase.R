...
#'The code is useful to plan a subsampling (called phase II) of a cohort/register (called phase I), on which to measure a new biomarker. Sampling design could be stratified for a variable available in the cohort (phase I e.g. by event or by possible counfounder/risk factor). If a stratification by an auxiliary variable of the new biomarker is considered, expected sensibility and specificity (as measures of accuracy of the auxiliary variable with respect to the new biomarker) in \code{acc.aux} argument need to be included.
#'Biomarker and stratum are assumed to be dichotomous variables.
...
#'@param betaBM expected beta coefficient (ln(HR))  of the new biomarker of interest on the time-to-event endpoint.
...
#'@export
#'@references Graziano, F., Valsecchi, M.G. & Rebora, P. Sampling strategies to evaluate the prognostic value of a new biomarker on a time-to-event end-point. BMC Med Res Methodol 21, 93 (2021). \url{https://doi.org/10.1186/s12874-021-01283-0}
#'@examples
#'\dontrun{
...
PowerIIphase<-function(pBM,betaBM, pstrata=NULL,betastrata=NULL,acc.aux=NULL,design2p=NULL,N,n,cens=0.1,tau=2,lambda=0.1, k=0.9,B=1000, seed=NULL)
  ...
  GG<-NULL;Performance<-NULL;Performance_d<-NULL;
    ...
    if(!missing(seed)) {
      ...
      if(is.null(acc.aux) & "CM" %in% design2p) {
        return("Missing accuracy of auxiliary!")
      }
      if(!is.null(pstrata) & is.null(betastrata)) {
        return("Missing beta of stratum!")
      }
      if(is.null(pstrata) & !is.null(betastrata)) {
        return("Missing prevalence of stratum!")
      }
